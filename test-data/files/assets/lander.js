...
window.JST={"atoms-button":"<a href=\"{{ url }}\" {{#action}}data-action=\"{{action}}\"{{/action}} class=\"btn t-bkg{{#invert}} invert{{/invert}}{{#btn-size}} btn--{{btn-size}}{{/btn-size}}{{#icon}} btn--with-icon{{/icon}}{{#transparent}} btn--transparent{{/transparent}}{{#btn-class}} {{btn-class}}{{/btn-class}}\"{{#disabled}} disabled{{/disabled}}>{{^icon-after}}{{# icon }}{{> atoms-icon}}{{/ icon }}{{/icon-after}} {{ btn-title }} {{#icon-after}}{{# icon }}{{> atoms-icon}}{{/ icon }}{{/icon-after}}</a>","atoms-icon":"<span class=\"icon icon-{{icon}} t-fill{{#color}} icon--{{color}}{{/color}}{{#icon-size}} icon--{{icon-size}}{{/icon-size}}{{#size}} icon--{{size}}{{/size}}{{#round}} icon-round{{/round}}{{#icon-class}} {{icon-class}}{{/icon-class}}\">\n  <svg role=\"img\">\n    <title>{{icon}}</title>\n    <use xlink:href=\"{{ icons-svg }}#{{icon}}\"></use>\n  </svg>\n</span>"}
var Lander,
...
Lander = (function() {
  ...
  Lander.prototype.default_options = {
    ...
    kaltura_product: typeof kWidget !== "undefined" && kWidget !== null,
    after_user_initiated_change: false,
    left_column_seek: false,
  ...
  function Lander(options, jsData) {
    this.generate_vid_and_meter_markup_kaltura = __bind(this.generate_vid_and_meter_markup_kaltura, this);
    ...
    this.userInitiatedKalturaPlay = __bind(this.userInitiatedKalturaPlay, this);
    this.kalturaPlayed = __bind(this.kalturaPlayed, this);
    this.kalturaPaused = __bind(this.kalturaPaused, this);
    this.playerSeekEndCallback = __bind(this.playerSeekEndCallback, this);
    this.playerUpdatePlayheadHandler = __bind(this.playerUpdatePlayheadHandler, this);
    ...
    if (this.options.kaltura_product) {
      this.article_el.prepend(this.generate_vid_and_meter_markup_kaltura(this.options["poster"]));
    } else {
    ...
    this.sectionsArray = [];
    this.pageLoadGuid = this.createGuid();
    analytics.track("oriole_page_load", {
      pageViewId: this.pageLoadGuid
    });
    this.measure_window();
    if (this.options.video_player) {
      ...
      if (this.options.kaltura_product) {
        $("<div id='player'></div>").prependTo('.article-video');
        video_player = $('#player');
      } else {
    ...
    if (this.options.kaltura_product) {
      kWidget.embed({
        "targetId": 'player',
        "wid": '_' + kalturaAccount,
        "uiconf_id": kalturaPlayer,
        ...
        readyCallback: function(playerId) {
          lander.kdp = $("#" + playerId).get(0);
          lander.kdp.addJsListener('playerUpdatePlayhead', 'lander.playerUpdatePlayheadHandler');
          lander.kdp.addJsListener('seeked', 'lander.playerSeekEndCallback');
          lander.kdp.addJsListener('playerPaused', 'lander.kalturaPaused');
          lander.kdp.addJsListener('playerPlayed', 'lander.kalturaPlayed');
          lander.kdp.addJsListener('userInitiatedPlay', 'lander.userInitiatedKalturaPlay');
          return lander.scale_video();
        }
      });
    }
  ...
  Lander.prototype.playerUpdatePlayheadHandler = function(data, id) {
    var c, nearest_cue, time, _i, _len, _ref;
    time = Math.round(data) * 1000;
    if (this.options.after_user_initiated_change) {
      nearest_cue = null;
      _ref = this.cues;
      ...
      this.options.after_user_initiated_change = false;
      return this.scroll_to(nearest_cue);
    } else {
      if (this.state.last_time !== time / 1000) {
        this.state.last_time = time / 1000;
        return this.events.trigger("timeupdate", time);
      }
    }
  };

  Lander.prototype.playerSeekEndCallback = function(seekedTime) {
    if (!this.options.left_column_seek) {
      this.options.after_user_initiated_change = true;
      setTimeout(function() {
        return lander.kdp.sendNotification('doPlay');
      }, 100);
    }
    return this.options.left_column_seek = false;
  };

  Lander.prototype.kalturaPaused = function() {
    this.state.playing = false;
    return this.controls.change(this.state);
  };

  Lander.prototype.kalturaPlayed = function() {
    this.state.playing = true;
    return this.state.video_has_played = true;
  };

  Lander.prototype.userInitiatedKalturaPlay = function() {
    return this.options.after_user_initiated_change = true;
  };

  Lander.prototype.createGuid = function() {
  ...
  Lander.prototype.setup_controls = function() {
    this.controls = new LanderControls(this.state, this.options, this.options.kaltura_product);
  ...
  Lander.prototype.set_and_setup_video = function(video_player) {
    ...
    if (!this.options.kaltura_product) {
      ...
      this.video_el = this.video_player;
    }
    ...
    return this.video_player.on("timeupdate", (function(_this) {
      return function(e) {
        ...
        if (_this.state.last_time !== time / 1000) {
          ...
          return _this.events.trigger("timeupdate", time);
        }
      };
    })(this));
  };
  ...
  Lander.prototype.setup_main_events = function() {
    ...
    this.events.on("scroll_threshold", (function(_this) {
      return function() {
        if (!_this.state.currently_autoscrolling) {
          if (!_this.options.kaltura_product) {
            return _this.pause();
          } else {
            return _this.kdp.sendNotification('doPause');
          }
        }
      };
    })(this));
    this.events.on("meter_click", (function(_this) {
      return function(y) {
        var cue_y, kaltura_video_duration, seconds_in, time;
        _this.options.left_column_seek = true;
        if (y < 0) {
          y = 0;
        }
        ...
        if (_this.options.kaltura_product) {
          _this.state.video_has_played = true;
          _this.state.playing = true;
          kaltura_video_duration = _this.kdp.evaluate('{duration}');
          if (kaltura_video_duration < seconds_in) {
            _this.kdp.sendNotification('doSeek', Math.floor(kaltura_video_duration - .5));
          } else {
            _this.kdp.sendNotification('doSeek', Math.floor(seconds_in));
          }
          return _this.kdp.sendNotification('doPlay');
        } else {
    ...
    this.events.on("timeupdate", (function(_this) {
      return function(time) {
        var video_progress_line;
        video_progress_line = _this.svg.select("line.video_progress").data([time]);
    ...
    this.events.on("user_jump", (function(_this) {
      return function() {
        ...
        if (!_this.options.kaltura_product) {
          ...
          _this.kalturaPlayed();
          _this.kdp.sendNotification('doSeek', milli / 1000);
        }
  ...
  Lander.prototype.measure_window = function() {
    ...
    if (!this.state.win || this.state.win.timestamp < new_win.timestamp - 10) {
      ...
      new_win.main_gutter_left = $('main').offset().left;
      new_win.main_gutter_right = new_win.width - new_win.main_width - new_win.main_gutter_left;
      new_win.header_height = $(this.options.header_selector).outerHeight();
      if (!this.state.win) {
  ...
  Lander.prototype.scale_video = function() {
    ...
    if (this.options.kaltura_product) {
      video_height = Math.max(this.options.min_video_height, this.state.win.height * this.options.video_height_ratio - this.state.win.scroll_top) + 33.96;
      video_width = (video_height - 33.96) * ratio;
    } else {
    ...
    if (video_width > this.state.win.main_width) {
      video_width = this.state.win.main_width;
      if (this.options.kaltura_product) {
        video_height = video_width / ratio + 33.96;
      } else {
        video_height = video_width / ratio;
      }
    }
    margin_right = (this.state.win.width - video_width) / 2;
    video_right = Math.max(this.state.win.main_gutter_right, margin_right - this.state.win.scroll_top * 1.5);
    ...
    this.video_background_el.css({
      ...
      left: this.state.win.main_gutter_left,
      top: video_top
    });
  ...
  Lander.prototype.hyperlink_tracking = function() {
    return $(this.options.link_tracking).on("click", function(event) {
      event = event.toElement;
      return analytics.track("link_clicked", {
        linkText: event.text,
        href: event.href,
        videoTitle: $('h1.title').text(),
        pageViewId: this.pageLoadGuid
      });
    });
  };

  Lander.prototype.unset_cues = function() {
    return $('.highlight').removeClass('highlight');
  };

  Lander.prototype.scroll_to = function(cue_or_pos) {
    var duration, pos, s, _i, _len, _ref;
    if (this.state.autoscroll_enabled !== true) {
      return;
    }
    this.find_cue_el_positions();
    ...
    _ref = this.sectionsArray;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      s = _ref[_i];
      if (s["keen"] === false) {
        if (s["section"]["sel"] === cue_or_pos["sel"]) {
          s["keen"] = true;
          analytics.track("section_reached", {
            sectionTitle: s["section"]["text"],
            selector: s["section"]["sel"],
            time: s["section"]["time"],
            sectionNumber: this.sectionsArray.indexOf(s) + 1,
            pageViewId: this.pageLoadGuid,
            janrainUuid: orm.Authenticator.currentUser()
          });
          break;
        }
      }
    ...
    duration = this.scales.cue_y_to_anim_dur(Math.abs(pos - this.state.win.scroll_top));
    return $("body,html").animate({
      scrollTop: pos
    }, duration, "swing", (function(_this) {
      return function() {
  ...
  Lander.prototype.setup_scales = function() {
    var cue_positions, cue_times, cues_with_pos, height, last_cue_date, width, _i, _ref, _ref1, _results;
    this.scales = {};
    ...
    last_cue_date = this.cues[this.cues.length - 1]["time"];
    this.scales.time_to_meter_y.domain([this.t_to_date([0]), last_cue_date]);
    ...
    this.scales.cue_y_to_cue_number = d3.scale.threshold().domain(_.pluck(cues_with_pos, 'pos')).range((function() {
      _results = [];
      for (var _i = 0, _ref1 = cues_with_pos.length; 0 <= _ref1 ? _i <= _ref1 : _i >= _ref1; 0 <= _ref1 ? _i++ : _i--){ _results.push(_i); }
      return _results;
    }).apply(this));
    return this.scales.cue_y_to_anim_dur = d3.scale.linear().domain([0, d3.max(cue_positions)]).range([500, 2000]);
  };
  ...
  Lander.prototype.setup_svg = function() {
    ...
    $(this.options.meter_selector).css("min-height", this.options.meter_height);
    _ref = this.get_meter_dims(), width = _ref[0], height = _ref[1];
  ...
  Lander.prototype.setup_markers_with_text = function() {
    ...
    return marks.selectAll("text, circle").on("click", (function(_this) {
      return function(d, b, i) {
        var kaltura_video_duration, seconds_in;
        seconds_in = _this.date_to_seconds(d["time"]);
        _this.options.left_column_seek = true;
        if (_this.options.kaltura_product) {
          kaltura_video_duration = _this.kdp.evaluate('{duration}');
          if (kaltura_video_duration < seconds_in) {
            _this.kdp.sendNotification('doSeek', kaltura_video_duration - .5);
          } else {
            _this.kdp.sendNotification('doSeek', seconds_in);
          }
  ...
  Lander.prototype.generate_vid_and_meter_markup_kaltura = function(poster) {
    return "<div class='article-video'> </div> <div id='meter'></div>";
  };
...
LanderControls = (function(_super) {
  ...
  LanderControls.prototype.template = '<div class="overlay" data-action="overlay"><div class="left"/><div class="center"/><div class="right"/></div><div class="under"></div>';

  LanderControls.prototype.kalturaTemplate = '<div class="kaltura-overlay" data-action="overlay"><div class="left"/><div class="center"/><div class="right"/></div><div class="under"></div>';

  LanderControls.prototype.defaults = {
    'icon-size': 'small'
  };

  LanderControls.prototype.initialize = function(state, options, kaltura_product) {
    this.data = _.extend({}, this.defaults, state, options, kaltura_product);
  ...
  LanderControls.prototype.renderChildren = function(html) {
    ...
    if (!this.data.kaltura_product) {
      controls.append(this.template);
      controls.append(this.createPosterImage());
    } else {
      if (!this.data.playing) {
        controls.append(this.kalturaTemplate);
      }
    }
    ...
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      ...
      if (child.position) {
        if (!this.data.kaltura_product) {
          ...
          controls.find(".kaltura-overlay ." + child.position).append(this.createButton(child));
        }
      } else {
        if (!this.data.kaltura_product) {
          controls.find(".overlay :nth-child(" + (i + 1) + ")").append(this.createButton(child));
        } else {
          controls.find(".kaltura-overlay :nth-child(" + (i + 1) + ")").append(this.createButton(child));
        }
      }
    }
    ...
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      child = _ref1[_j];
      controls.find(".under").append(this.createButton(child));
    }
  ...
  LanderControls.prototype.update = function() {
    ...
    if (!this.data.playing) {
      if (this.data.win.scroll_top > this.data.top_threshold) {
        ...
        if (!this.data.kaltura_product && this.data.last_time > 10) {
          this.data.overlay.push(this.buttons.restart);
        }
      }
    }
    if (!this.data.kaltura_product) {
      if (this.data.video_has_played && !this.data.playing) {
        this.data.overlay.push(this.buttons.resume);
      } else if (!this.data.video_has_played) {
    ...
    if (this.data.autoscroll_enabled && !this.data.playing) {
      ...
      if (this.data.show_feedback) {
        return this.data.under.push(this.buttons.feedback);
      }
    } else if (!this.data.autoscroll_enabled) {
      this.data.under.push(this.buttons.autoscroll_unchecked);
      return this.data.under.push(this.buttons.feedback);
    }
  };
  ...
  LanderControls.prototype.setupEvents = function() {
    var ev, _fn, _fn1, _i, _j, _len, _len1, _ref, _ref1;
    if (this.data.kaltura_product) {
      _ref = ["autoscroll", "open-oriole-feedback", "jump"];
      _fn = (function(_this) {
        return function(ev) {
      ...
      _ref1 = ["play", "pause", "jump", "rewind", "next", "overlay", "autoscroll", "open-oriole-feedback"];
      _fn1 = (function(_this) {
        return function(ev) {
          return _this.$el.on("click", "[data-action='" + ev + "']", function(e) {
            _this.trigger(ev, e);
  ...
  LanderControls.prototype.buttons = {
    ...
    feedback: {
      html: "<div  class='send-feedback'><a href='#' data-action='open-oriole-feedback' class='nice-underline'>Let us know what you think!</a></div>"
    },
    autoscroll_unchecked: {
      html: "<div class='autoscroll'><input name='autoscroll' type='checkbox' data-action='autoscroll'> <label data-action='autoscroll' for='autoscroll'>auto scroll/pause</label></div>"
    },
    autoscroll_checked: {
      html: "<div class='autoscroll'><input checked name='autoscroll' type='checkbox' data-action='autoscroll'> <label data-action='autoscroll' for='autoscroll'>auto scroll/pause</label></div>"
    },
    jump: {
      action: 'jump',
      template: 'atoms-button',
